...
parent      = new HBox(1 CURSORdeparrComboBox null, viewComboBox HBox null) # -1,
parent span = <6304..8174>,
child       = viewComboBox HBox null # -1,
child span  = [6336..6355..8179]
...
object Dashboard extends JFXApp3:
  ...
  def start() =
    ...
    def initializeGraph(): VBox = {
      ...
      val comboBoxHBox1 = new HBox(10, graphComboBox, datasetComboBoxC)
      val comboBoxHBox2 = new HBox(1@@deparrComboBox, viewComboBox
      HBox.clearConstraints(graphComboBox)
      HBox.clearConstraints(datasetComboBoxC)
      HBox.clearConstraints(deparrComboBox)
      HBox.clearConstraints(viewComboBox)       
      val comboBoxHBox2 = new HBox(10, datasetComboBoxM, deparrComboBox, viewComboBox)
           
      val chartVBox = new VBox()
      ...
      def updateChart(): Unit = {
          ...
          if selectedView == "Graph View" then
            val newChart = makeChart(selectedGraph, getChartData(selectedDatasetC)._1, getChartData(selectedDatasetC)._2, getChartData(selectedDatasetC)._3, getChartData(selectedDatasetC)._4)
            chartVBox.children.clear()
            chartVBox.children.addAll(newChart)
            v = 0
          else
            val newMetric = card.makeMetric(getMetricData(selectedDatasetM)._1, getMetricData(selectedDatasetC)._2)
            chartVBox.children.clear()
            chartVBox.children.addAll(newMetric)
            v = 1
        }
