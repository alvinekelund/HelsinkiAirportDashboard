...
parent span = <10545..11513>,
child       = def run(): Unit =
  {
    Platform.runLater(
      () =>
        if viewComboBox.value.value == "Chart View" then updateChart() else null
      )
  } # -1,
child span  = [10571..10575..11515]
...
object Dashboard extends JFXApp3:
  ...
  def start() =
    ...
    defaultChart.getChildren().addAll(makeChart("Pie", graphData.flightPerHourData(getAllFlightData()), "Time", "Airplanes flown", "Airplanes flown each hour"))
