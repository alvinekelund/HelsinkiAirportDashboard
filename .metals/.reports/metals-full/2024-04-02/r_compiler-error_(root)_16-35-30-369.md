...
class Metric {
  def makeMetric(name: String, initialValue: String): Node = {
    ...
    val closeButton = new SVGPath {
      content = "M 0 0 L 10 10 M 0 10 L 10 0" // Cross shape
      fill = Color.Black
      translateX = rect.width() / 2 - 15
      translateY = -rect.height() / 2 + 5
      onMouseClicked = (event: MouseEvent) => removeCard(card)
    }

    val card = new StackPane
    card.children.addAll(rect, vbox, closeButton)
    ...
    card.onMouseDragged = (event: MouseEvent) => {
      val offsetX = event.sceneX - orgSceneX
      val offsetY = event.sceneY - orgSceneY
  ...
  private def removeCard(card: <S@@): Unit = {
    val parent = card.parent.value
